...
#ifdef DEBUG_IOPINS    
  pinMode(DEBUG_PIN_STEP, OUTPUT);
  pinMode(DEBUG_PIN_COMMIT, OUTPUT);
...
void loop(void)
{
  ...
  if (g_InControlState.fRobotOn) {
    ...
    if (g_InControlState.fPrev_RobotOn || (AllDown= 0)) {
      ...
      Serial.println("FreeServos called");
  ...
  //Store previous XBeeSerial.available() State
  if (g_InControlState.fRobotOn)
...
#elif defined(OCTOMODE)
    (LegPosY[cRMR]==(short)cInitPosY[cRMR]) && 
    (LegPosY[cRMF]==(short)cInitPosY[cRMF]) && 
    (LegPosY[cLMR]==(short)cInitPosY[cLMR]) && 
    (LegPosY[cLMF]==(short)cInitPosY[cLMF]) && 
#endif  
  ...
  if (g_InControlState.SelectedLeg<= (CNT_LEGS-1)) {
    if (g_InControlState.SelectedLeg!=g_InControlState.PrevSelectedLeg) {
      if (AllDown) { //Lift leg a bit when it got selected
        ...
        LegPosX[g_InControlState.PrevSelectedLeg] = (short)cInitPosX[g_InControlState.PrevSelectedLeg];
        LegPosY[g_InControlState.PrevSelectedLeg] = (short)cInitPosY[g_InControlState.PrevSelectedLeg];
        LegPosZ[g_InControlState.PrevSelectedLeg] = (short)cInitPosZ[g_InControlState.PrevSelectedLeg];
      }
    } 
...
#else
short CheckServoAngleBounds(short sID __attribute__((unused)),  short sVal, const short *sMin, const short *sMax) {
#endif
