...
void cm904Controller::setup(int servo_cnt, const char *portName, uint8_t protocol, uint32_t baud) {
  int i;
  // setup storage
  ...
  portHandler[0] = dynamixel::PortHandler::getPortHandler(portName);
  // Initialize packetHandler instance
  // Set the protocol version
  // Get methods and members of Protocol1PacketHandler or Protocol2PacketHandler
  packetHandler[0] = dynamixel::PacketHandler::getPacketHandler((protocol==2)? 2.0 : 1.0);
  // Open port
  if (!portHandler[0]->openPort()) {
#ifdef DBGSerial
    DBGSerial.print("Failed to open the Dynamixel port!\n");
#endif
    return;
  }
  if (!portHandler[0]->setBaudRate(baud)) {
#ifdef DBGSerial
